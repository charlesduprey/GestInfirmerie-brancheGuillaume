	...
	public partial class frmImport : Form
	{
		...
		private bool CreateTableInDatabase(OdbcDataReader dtSchemaTable, string tableOwner, string tableName, string connectionString)
		{
			try
			{
                ...
                //**********************************
                string ctStr = "INSERT INTO [" + tableOwner + "].[" + tableName + "] (archive_elv,nom,prenom,date_naissance,tel_eleve,tel_parent,tier_temps,commentaire_sante,id_classe) VALUES ('0','";
                string controlNom = " SELECT count(*) FROM ELEVES WHERE UPPER(nom) = '";
                string updateId_classe = "UPDATE ELEVES SET archive_elv ='0', id_classe = '";
                string reset_archive = "UPDATE ELEVES SET archive_elv = '1'";
                int valRet = 0;

                //***********************************************
                // initialisation des attributs archive_elv à 1
                //**********************************************
                ...
                while (dtSchemaTable.Read())
                {
                    ...
                    ctStr = "INSERT INTO [" + tableOwner + "].[" + tableName + "] (archive_elv,nom,prenom,date_naissance,tel_eleve,tel_parent,tier_temps,commentaire_sante,id_classe) VALUES ('0','";
                    controlNom = " SELECT count(*) FROM ELEVES WHERE UPPER(nom) = '";
                    updateId_classe = "UPDATE ELEVES SET archive_elv ='0', id_classe = '";
                }
                return true;
